...
def get_info():
    return jsonify(
        descricao = "gerenciamento de comentarios da Livraria Collins",
        versao = "1.0"
    )

@servico.get("/comentarios/<int:id_do_livro>")
def get_comentarios(id_do_livro):
    comentarios = []

    conexao = get_conexao_com_bd()
    cursor = conexao.cursor(dictionary=True)
    cursor.execute("SELECT id, comentario, nome, conta, DATE_FORMAT(data, '%Y-%m-%d %H:%i') AS data " +
                    "FROM comentarios " +
                    "WHERE id_livro = " + str(id_do_livro))
...
@servico.post("/adicionar/<int:id_do_livro>/<string:nome>/<string:conta>/<string:comentario>")
def add_comentario(id_do_livro, nome, conta, comentario):
    ...
    try:
        cursor.execute(
            f"INSERT INTO comentarios(id_livro, nome, conta, comentario, data) VALUES({id_do_livro}, '{nome}', '{conta}', '{comentario}', NOW())")
        conexao.commit()
    except:
        conexao.rollback()
